...
class BaseVision(Model):
    def __init__(self, config):
        ...
        if config.model_vision_attention == 'position':
            ...
            # 入力画像は dataset_image_(height,width)。resnet45 出力は概ね /4 になる前提。
            feat_h = ifnone(config.model_vision_feat_h, config.dataset_image_height // 4)
            feat_w = ifnone(config.model_vision_feat_w, config.dataset_image_width // 4)
