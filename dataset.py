...
class SpellingMutation(object):
    def __init__(self, pn0=0.7, pn1=0.85, pn2=0.95, pt0=0.7, pt1=0.85, charset=None):
        ...
        logging.info(f"the probs: pn0={self.pn0}, pn1={self.pn1} " + f"pn2={self.pn2}, pt0={self.pt0}, pt1={self.pt1}")
...
class HuggingFaceTextDataset(Dataset):
    """Hugging Faceデータセットからテキストデータを読み込むDatasetクラス。

    TextDatasetと同様のインターフェースを提供し、Hugging Face Hubのデータセットを
    直接読み込むことができます。複数のデータセットを結合することも可能です。
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Args:
            dataset_names: Hugging Faceデータセット名のリスト (例: ['Kotomiya07/honkoku-hq', 'Kotomiya07/honkoku-v3.0'])
            text_column: テキストが含まれるカラム名 (デフォルト: 'text')
            max_length: 最大テキスト長
            charset_path: 文字セットファイルのパス
            case_sensitive: 大文字小文字を区別するか
            one_hot_x: 入力にone-hotエンコーディングを使用するか
            one_hot_y: 出力にone-hotエンコーディングを使用するか
            is_training: 訓練モードか
            smooth_label: ラベルスムージングを使用するか
            smooth_factor: ラベルスムージングの係数
            use_sm: SpellingMutationを使用するか
            split: データセットの分割名 (デフォルト: 'train')
        """
        ...
        # Hugging Faceデータセットを読み込んで結合
        datasets = []
        for name in self.dataset_names:
            logging.info(f"Loading Hugging Face dataset: {name} (split={split})")
            try:
                ds = load_dataset(name, split=split)
                datasets.append(ds)
                logging.info(f"Loaded {len(ds)} samples from {name}")
            except Exception as e:
                logging.warning(f"Failed to load {name} with split={split}: {e}")
                # splitが存在しない場合は、利用可能なsplitを試す
                try:
                    ds_info = load_dataset(name, split=None)
                    available_splits = list(ds_info.keys())
                    logging.info(f"Available splits for {name}: {available_splits}")
                    if available_splits:
                        # train, train+validation, または最初の利用可能なsplitを使用
                        preferred_splits = ["train", "train+validation", available_splits[0]]
                        for preferred_split in preferred_splits:
                            if preferred_split in available_splits:
                                ds = ds_info[preferred_split]
                                datasets.append(ds)
                                logging.info(f"Loaded {len(ds)} samples from {name} (split={preferred_split})")
                                break
                except Exception as e2:
                    logging.error(f"Failed to load {name}: {e2}")
                    raise

        if not datasets:
            raise ValueError(f"No datasets were successfully loaded from {self.dataset_names}")

        # データセットを結合
        if len(datasets) > 1:
            self.dataset = concatenate_datasets(datasets)
            logging.info(f"Concatenated {len(datasets)} datasets, total samples: {len(self.dataset)}")
        else:
            self.dataset = datasets[0]
            logging.info(f"Total samples: {len(self.dataset)}")
    ...
    def __getitem__(self, idx):
        ...
        # 最大長をチェック
        if len(text_x) > self.charset.max_length - 1:
            text_x = text_x[: self.charset.max_length - 1]
        ...
        # 言語モデルの事前学習では、入力と出力は同じテキスト（自己回帰学習）
        # text_yは既に改行が除去されたtextを使用
        text_y = text.strip()
        # text_y = re.sub('[^0-9a-zA-Z]+', '', text_y)
        if not self.case_sensitive:
            text_y = text_y.lower()
        ...
        # 最大長をチェック
        if len(text_y) > self.charset.max_length - 1:
            text_y = text_y[: self.charset.max_length - 1]
